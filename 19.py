# mylist = []
# with open("19-2-input.txt", "r") as f:  # log.txt file has line separated values,
...
#
# rules = []
# with open("19-test-input.txt", "r") as f:  # log.txt file has line separated values,
#     for i in f.readlines():
#         i = i.strip()
#         rules.append(i)
# print(rules)
...
#     colon = rule.find(':')
#     bar = rule.find('|')
#     ruleNum = rule[0:colon]
#     if bar > 0:
#         rule1 = rule[colon + 2:bar - 1]
#         rule2 = rule[bar + 2:]
#         ruleDict[ruleNum] = (rule1.split(' '), rule2.split(' '))
#     else:
#         rule1 = rule[colon + 2:]
#         ruleDict[ruleNum] = (rule1.split(' '),)
#
# print(ruleDict['0'])
#
#
# def recurseUpdate(ruleNum):
#     updatedOptions = ()
#     for index, option in enumerate(ruleDict[ruleNum]):
#         for rule in option:
#             rule = ruleDict[rule]
#     pass
#
#
# legalMessages = ruleDict[0]
#
# for
...
def part2():
    ...
    for message in messages:
        if len(message) % ruleLen != 0 or len(message) < (ruleLen * 3):
            ...
            if isValid(rule42, message[0:ruleLen]) and isValid(rule42, message[ruleLen:2 * ruleLen]) and isValid(rule31,
                                                                                                                 message[
                                                                                                                 -ruleLen:]):
                midSegs = int(len(message) / ruleLen) - 3
                valid42 = all(isValid(rule42, message[(2 + i) * ruleLen:(3 + i) * ruleLen]) for i in
                              range(int(midSegs / 2) + midSegs % 2))
...
rulesDict, messages = makeRules("19-test-input.txt")
rule0 = getRule(0)
print(getLen(rule0))
